...
export async function GET(request) {
  try {
    ...
    const apiUrl = `${apiBaseUrl}/get-data-daily.json?session=${encodeURIComponent(decodedSession)}&id=${accountId}&start=${startDate}&end=${endDate}`;
      ...
      return NextResponse.json(
        {
          success: false,
          error: true,
          message: data.message || "Failed to retrieve daily data",
          dataDaily: [],
        },
        { status: 401 }
      );
    ...
    console.error("Error in MyFxBook get daily data:", error);
    return NextResponse.json(
      {
        success: false,
        error: true,
