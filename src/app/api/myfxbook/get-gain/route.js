  ...
  try {
    ...
    if (!session) {
      try {
        ...
        console.log("Auto-logged in to get session for get-gain (GET)");
      } catch (error) {
        return NextResponse.json(
          {
            success: false,
            error: true,
            message: `Failed to get session for gain: ${error.message}`,
            value: null,
          },
          { status: 500 }
        );
    ...
    const apiUrl = `${apiBaseUrl}/get-gain.json?session=${encodeURIComponent(decodedSession)}&id=${accountId}&start=${startDate}&end=${endDate}`;
    ...
    if (data.error === false && data.value !== undefined) {
      ...
      return NextResponse.json({
        success: true,
        error: false,
        message: data.message || "Gain retrieved successfully",
        value: data.value,
      });
    } else {
      return NextResponse.json(
        {
          success: false,
          error: true,
          message: data.message || "Failed to retrieve gain",
          value: null,
        },
        { status: 401 }
      );
    ...
    console.error("Error in MyFxBook get gain:", error);
    return NextResponse.json(
      {
        success: false,
        error: true,
...
export async function POST(request) {
  try {
    ...
    if (!session) {
      try {
        session = await getSessionFromLogin();
        console.log("Auto-logged in to get session for get-gain (POST)");
      } catch (error) {
        return NextResponse.json(
          {
            success: false,
